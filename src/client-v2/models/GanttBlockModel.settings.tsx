...
export function registerGanttBlockModelSettings(GanttBlockModel: any) {
  GanttBlockModel.registerFlow({
    ...
    steps: {
      ...
      allowUserZoom: {
        title: tExpr('Allow users to customize zoom'),
        uiMode: { type: 'switch', key: 'allowUserZoom' },
        defaultParams(ctx) {
          const model = getGanttModel(ctx);
          return { allowUserZoom: model.props?.allowUserZoom === true };
        },
        handler(ctx, params) {
          getGanttModel(ctx).setProps('allowUserZoom', params.allowUserZoom === true);
        },
        beforeParamsSave(ctx, params) {
          getGanttModel(ctx).setProps('allowUserZoom', params.allowUserZoom === true);
        },
