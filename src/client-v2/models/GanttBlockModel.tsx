...
export class GanttBlockModel extends TableBlockModel {
  ...
  renderTimeNavigation() {
    return (
      <Space size={0}>
        <Button icon={<LeftOutlined />} onClick={() => this.emitter.emit('timeNavigate', 'left')} />
        <Button onClick={() => this.emitter.emit('timeNavigate', 'today')}>
          {this.getTodayLabel(this.getUserViewMode())}
        </Button>
        <Button icon={<RightOutlined />} onClick={() => this.emitter.emit('timeNavigate', 'right')} />
      </Space>
    );
  }
