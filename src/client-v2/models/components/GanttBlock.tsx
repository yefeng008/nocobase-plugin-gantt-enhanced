...
import { addToDate, ganttDateRange, seedDates } from '../../shared/helpers/date-helper';
...
export const GanttBlock = observer(
  ({ model }: { model: GanttBlockModel }) => {
    ...
    const [viewMode, setViewMode] = useState(() => model.getUserViewMode());
    ...
    useEffect(() => {
      ...
      model.emitter.on('viewModeChange', handleViewModeChange);
      return () => {
        model.emitter.off('viewModeChange', handleViewModeChange);
      };
    }, [model]);

    useEffect(() => {
      const getViewModeStep = (mode: string): { quantity: number; scale: 'hour' | 'day' | 'month' | 'year' } => {
        switch (mode) {
          case 'hour':
            return { quantity: 1, scale: 'hour' };
          case 'quarterDay':
            return { quantity: 6, scale: 'hour' };
          case 'halfDay':
            return { quantity: 12, scale: 'hour' };
          case 'week':
            return { quantity: 7, scale: 'day' };
          case 'month':
            return { quantity: 1, scale: 'month' };
          case 'quarterYear':
            return { quantity: 3, scale: 'month' };
          case 'year':
            return { quantity: 1, scale: 'year' };
          default:
            return { quantity: 1, scale: 'day' };
        }
      };

      const rebuildBars = (newDates: Date[]) => {
        setDateSetup({ viewMode, dates: newDates });
        setBarTasks(
          convertToBarTasks(
            visibleTasks,
            newDates,
      ...
      const handleTimeNavigate = (direction: 'left' | 'right' | 'today') => {
        const { quantity, scale } = getViewModeStep(viewMode);
        let newDates: Date[];
        if (direction === 'today') {
          const now = new Date();
          ...
          for (let i = 0; i < count; i++) {
            ...
            current = addToDate(current, quantity, scale);
          }
        } else {
      ...
      model.emitter.on('timeNavigate', handleTimeNavigate);
      return () => {
        model.emitter.off('timeNavigate', handleTimeNavigate);
      };
    }, [
      model,
      viewMode,
